function [model,predictions]=MTRL(init, data,label,kertype,kerpar,lambda1,lambda2,testdata)
%Input:
...
%      predictions: a 1*m cell array when 'testdata' is not empty. Each cell contains a row vector, which is the predictive function values for one task.
    ...
    max_iteration=30 ;                                                     %%% Kaixiang 
    threshold=10^(-12);                                                     %%% Kaixiang 
...
%     old_W = CalculateW(data,model,task_index,m,Omega,lambda1,lambda2);
    ...
    for t=1:max_iteration
        ...
        m_Cor=eigVector*diag(eigValue./(lambda1*eigValue+lambda2))*eigVector';% Omega(lambda_1*Omega + lambda_2*Im)^{-1} plugin eigen decomposition.
