...
function model = MTRL_leastsquare_Wb(X,Y,model,parameters,Fista_options)
...
while iter < maxIter
    ...
    fg_yk  = gradVal_eval (yk,X,Y,d,K,lambda1); % gradient
    fv_yk  = funVal_eval  (yk,X,Y,d,K,lambda1); % function value
...
%         next_point = yk-1/Lk*fg_yk;
        ...
        Fv_plyk = funVal_eval(pL_yk,X,Y,d,K,lambda1);
        delta   = pL_yk - yk;
...
model.W = W_new;
model.b = b_new;
model.funcval = funcVal;

% private functions


    % gradient of smooth part f at a given point x.
    function [grad_x] = gradVal_eval(x,X,Y,d,K,lambda1)
        % gradient goes here
         ...
         grad_W = grad_W + W*(lambda1*eye(K));   
         grad_x = [grad_W(:);grad_b];
         
    end

    % function value of smooth part f.
    function [funcVal] = funVal_eval (x,X,Y,d,K,lambda1)
...
%         W = reshape(x,d,K);
        ...
        for i = 1:K
         ...
         funcVal = funcVal + lambda1/2*trace(W'*W);
    end
