function [trainX,trainY,validX,validY,trainallX, trainallY, testX,testY] = split_data_zscoreXY(data,label,ratio)
...
for i = 1:K
    ...
    Yi           = Y(trainall_index,:);
    [Yi_zscored,Yimean,Yistdev] = zscore(Yi);
    
    trainallY{i} = Yi_zscored;
    
    
    trainX{i} = X(train_index,:);
    trainY{i} = (Y(train_index,:)-Yimean)/Yistdev;
    
    validX{i} = X(valid_index,:);
    validY{i} = (Y(valid_index,:)-Yimean)/Yistdev;
    
    testX{i}  = X(test_index,:);
    testY{i}  = (Y(test_index,:)-Yimean)/Yistdev;
